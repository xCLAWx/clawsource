...
function sendVoice(chat_id,reply_id,voice,capt
pcall(tdcli_function({
ID="SendMessage",
chat_id_ = chat_id,
reply_to_message_id_ = reply_id,
...
if #list==0 then  return  "•**لا يوجد ادمنيه في هذه المجموعه " end
...
sendMsg(msg.chat_id_,msg.id_,'•*The group has been activated . \n*•And all cities were raised .\n@xCLAWx')
else
sendMsg(msg.chat_id_,msg.id_,'•تـم تـفـعـيـل المـجمـوعه‏‏ .')
end
GetChannelAdministrators(msg.chat_id_,function(arg,data)
for k,v in pairs(data.members_) do
...
return SendMention(ChatID,UserID,MsgID,'•العضو > ❪ '..USERNAME..' ❫\n•الايدي > ❪ '..UserID..' ❫\n•انه بالتأكيد منشىء اساسي .',17,USERCAR) 
end
redis:hset(max..'username:'..UserID,'username',Resolv)
redis:sadd(max..':KARA_BOT:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'•العضو > ❪ '..USERNAME..' ❫\n•الايدي > ❪ '..UserID..' ❫\n•تمت ترقيته ليصبح منشىء اساسي .',17,USERCAR) 
end

if cmd == "remkara" then
if not redis:sismember(max..':KARA_BOT:'..ChatID,UserID) then
return SendMention(ChatID,UserID,MsgID,'•العضو > ❪ '..USERNAME..' ❫\n•الايدي > ❪ '..UserID..' ❫\n•انه بالتأكيد ليس منشىء اساسي .',17,USERCAR) 
end
redis:srem(max..':KARA_BOT:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'•العضو > ❪ '..USERNAME..' ❫\n•الايدي > ❪ '..UserID..' ❫\n•تم تنزيله من المنشىء الاساسي .',17,USERCAR) 
end
...
return SendMention(ChatID,UserID,MsgID,'•العضو > ❪ '..USERNAME..' ❫\n•الايدي > ❪ '..UserID..' ❫\n•انه بالتأكيد ادمن .',17,USERCAR) 
end
redis:hset(max..'username:'..UserID, 'username', Resolv)
redis:sadd(max..'admins:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'•العضو > ❪ '..USERNAME..' ❫\n•الايدي > ❪ '..UserID..' ❫\n•تمت ترقيته ليصبح ادمن .',17,USERCAR) 
end
...
return SendMention(ChatID,UserID,MsgID,'•العضو > ❪ '..USERNAME..' ❫\n•الايدي > ❪ '..UserID..' ❫\n•انه بالتأكيد ليس ادمن .',17,USERCAR) 
end
redis:srem(max..'admins:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'•العضو > ❪ '..USERNAME..' ❫\n•الايدي > ❪ '..UserID..' ❫\n•تم تنزيله من الادمنيه .',17,USERCAR) 
end
